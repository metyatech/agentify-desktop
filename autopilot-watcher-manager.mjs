...
export function createAutopilotWatcherManager({ root, readFile = fs.readFile, lstat = fs.lstat, unlink = fs.unlink, spawnImpl = spawn, isPidAlive = (pid) => { try { process.kill(pid, 0); return true; } catch { return false; } }, platform = process.platform, env = process.env } = {}) {
  if (!root) throw new TypeError('watcher root is required');
  const paths = watcherPaths(root);
  let child = null;
  let current = { status: 'offline', detail: 'Watcher is not configured.' };
  ...
  const start = async () => {
    ...
    if (child && child.exitCode === null) return { status: 'starting', detail: 'Watcher is starting.' };
